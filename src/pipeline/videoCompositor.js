...
async function composeVideo({
  ...
  // Download any remote URLs (custom assets) to local files
  for (const entry of timelineEntries) {
    for (let idx = 0; idx < entry.sourcePaths.length; idx++) {
      const sp = entry.sourcePaths[idx];
      if (sp.startsWith('http://') || sp.startsWith('https://')) {
        const ext = entry.isVideo ? 'mp4' : 'png';
        const localPath = path.join(workDir, `custom_asset_${entry.order}_${idx}.${ext}`);
  ...
  for (let i = 0; i < timelineEntries.length; i++) {
    ...
    if (entry.isCustomAsset && entry.isVideo) {
      // Custom uploaded video — normalize to 1920x1080 and trim/loop to target duration
      // Preserve audio if useUploadedAudio is true (user left narration blank)
      console.log(`[Compositor] Custom video clip: useUploadedAudio=${entry.useUploadedAudio}`);
      await normalizeVideoClip(entry.sourcePath, clipPath, entry.duration, false, false, entry.useUploadedAudio);
    } else if (entry.isCustomAsset && !entry.isVideo) {
      // Custom uploaded image — create a video from the still image with Ken Burns effect
      await imageToVideo(entry.sourcePath, clipPath, entry.duration);
    } else if (entry.isVideo && isBrollEntry && entry.sourcePaths.length > 1) {
...
function buildTimeline(segments, timestamps, sceneImages, brollImages) {
  ...
  for (const seg of segments) {
    ...
    // Custom asset (uploaded image/video)
    if (seg.visualType === 'custom_asset' && seg.customAssetUrl) {
      // Custom assets are downloaded during compositing (handled in composeSingleSegment)
      sourcePaths = [seg.customAssetUrl];
      isVideo = seg.customAssetType === 'video';
    }
    // Scene capture (now video clips)
    else if (seg.visualType === 'scene_capture' && seg.sceneId && sceneImages[seg.sceneId]) {
    ...
    entries.push({
      ...
      useUploadedAudio: !!(seg.useUploadedAudio && seg.visualType === 'custom_asset'),
      isCustomAsset: seg.visualType === 'custom_asset',
    });
  }
 ...
 */
async function normalizeVideoClip(inputPath, outputPath, targetDuration, freezeIfShort = false, forceLoop = false, preserveAudio = false) {
  const clipDuration = await getVideoDuration(inputPath);

  // Custom asset with uploaded audio — normalize video AND keep audio track
  if (preserveAudio) {
    console.log(`[Compositor] Custom clip with audio: ${clipDuration.toFixed(1)}s → target ${targetDuration.toFixed(1)}s (preserving audio)`);
    return runFfmpeg([
      '-y',
      '-i', inputPath,
      ...
      '-movflags', '+faststart',
      outputPath,
    ], `normalizing custom clip with audio (${targetDuration.toFixed(1)}s)`);
  }
 ...
 */
function videoHasAudioTrack(videoPath) {
...
async function overlayAudio(videoPath, audioPath, outputPath, onProgress, musicPath) {
  ...
  if (musicPath && fs.existsSync(musicPath)) {
    ...
    // Also include video's embedded audio if present (from custom asset segments)
    const hasVideoAudio = await videoHasAudioTrack(videoPath);
    if (hasVideoAudio) {
      return runFfmpeg([
        '-y',
        '-i', videoPath,
        ...
        '-filter_complex',
        '[0:a]volume=1.0[vid_audio];[1:a]volume=1.0[voice];[2:a]volume=0.12[music];[vid_audio][voice][music]amix=inputs=3:duration=longest:dropout_transition=3[aout]',
        '-map', '0:v:0',
        '-map', '[aout]',
        '-c:v', 'copy',
        '-c:a', 'aac', '-b:a', '192k',
        ...
        '-movflags', '+faststart',
        outputPath,
      ], 'audio overlay + music + custom asset audio mix');
    } else {
      return runFfmpeg([
        '-y',
        '-i', videoPath,
    ...
    // Check if the concatenated video has an audio track (from custom assets with uploaded audio)
    const hasVideoAudio = await videoHasAudioTrack(videoPath);
    if (hasVideoAudio) {
      // Mix voiceover with embedded audio from custom asset segments
      return runFfmpeg([
        '-y',
        '-i', videoPath,
        '-i', audioPath,
        '-filter_complex',
        '[0:a]volume=1.0[vid_audio];[1:a]volume=1.0[voice];[vid_audio][voice]amix=inputs=2:duration=longest:dropout_transition=3[aout]',
        '-map', '0:v:0',
        '-map', '[aout]',
        '-c:v', 'copy',
        '-c:a', 'aac', '-b:a', '192k',
        ...
        '-movflags', '+faststart',
        outputPath,
      ], 'audio overlay (voiceover + custom asset audio mix)');
    } else {
      // Video has no audio — just overlay voiceover
      return runFfmpeg([
        '-y',
        '-i', videoPath,
        '-i', audioPath,
